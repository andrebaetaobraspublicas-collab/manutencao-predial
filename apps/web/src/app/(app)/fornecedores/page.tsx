...
import { Pencil, Plus, Save, ShieldAlert, Trash2, UsersRound, X } from 'lucide-react';
...
export default function SuppliersPage() {
  ...
  const load = useCallback(async () => { try { const [suppliers, catalog] = await Promise.all([
    apiFetch<Supplier[]>('/suppliers'), apiFetch<CatalogItem[]>('/operations/catalogs?kind=SPECIALTY&activeOnly=true')]);
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSubmitting(true); setError(''); try {
    await apiFetch(editingId ? `/suppliers/${editingId}` : '/suppliers', { method: editingId ? 'PATCH' : 'POST', body: JSON.stringify({ ...form,
    ...
    }) }); setForm(EMPTY); setEditingId(null); setShowForm(false); setLoading(true); await load();
  ...
  function editSupplier(item: Supplier) {
    setEditingId(item.id); setSelected(item); setShowForm(true);
    setForm({ kind: item.kind, legalName: item.legalName, tradeName: item.tradeName ?? '', taxId: item.taxId,
      email: item.email ?? '', phone: item.phone ?? '', contactName: item.contactName ?? '',
      addressLine1: item.addressLine1 ?? '', addressLine2: item.addressLine2 ?? '', district: item.district ?? '',
      city: item.city ?? '', state: item.state ?? '', postalCode: item.postalCode ?? '', notes: item.notes ?? '',
      serviceAreaCategoryIds: item.serviceAreaLinks?.map((entry) => entry.category.id) ?? [],
      memberIds: item.consortiumMembers?.map((entry) => entry.member.id) ?? [] });
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }

  async function archiveSupplier(item: Supplier) {
    if (!window.confirm(`Excluir ${item.tradeName || item.legalName}? O cadastro será arquivado e o histórico permanecerá disponível nos registros vinculados.`)) return;
    setError('');
    try { await apiFetch(`/suppliers/${item.id}`, { method: 'DELETE' }); if (selected?.id === item.id) setSelected(null); await load(); }
    catch (cause) { setError(cause instanceof ApiError ? cause.message : 'Não foi possível excluir o fornecedor.'); }
  }

  const companies = items.filter((item) => item.kind === 'COMPANY');
  return <div className="page-container"><header className="page-header"><div className="page-title"><h1>Fornecedores</h1><p>Empresas, consórcios, especialidades cadastradas e histórico de sanções.</p></div><button className="btn btn-primary" type="button" onClick={() => { if (showForm) { setForm(EMPTY); setEditingId(null); } setShowForm((value) => !value); }}>{showForm ? <X size={16} /> : <Plus size={16} />}{showForm ? 'Fechar cadastro' : 'Novo fornecedor'}</button></header>
  {error ? <div className="notice error" style={{ marginBottom: 18 }}>{error}</div> : null}
  {showForm ? <form className="card form-card" onSubmit={submit} style={{ marginBottom: 18 }}><section className="form-section"><div className="form-section-header"><h2>{editingId ? 'Editar fornecedor' : 'Dados cadastrais'}</h2><p>As áreas de atuação são carregadas automaticamente das especialidades da configuração operacional.</p></div><div className="form-grid">
    ...
    <F c="col-12" l="Áreas de atuação"><details className="input" style={{ height: 'auto', minHeight: 42 }}><summary>{form.serviceAreaCategoryIds.length ? `${form.serviceAreaCategoryIds.length} especialidade(s) selecionada(s)` : 'Selecione uma ou mais especialidades'}</summary><div style={{ display: 'grid', gap: 8, padding: '12px 4px 4px' }}>{categories.map((specialty) => <label key={specialty.id} style={{ display: 'flex', gap: 8 }}><input type="checkbox" checked={form.serviceAreaCategoryIds.includes(specialty.id)} onChange={() => setForm({ ...form, serviceAreaCategoryIds: toggle(form.serviceAreaCategoryIds, specialty.id) })} /> {specialty.code} — {specialty.name}</label>)}{!categories.length ? <span className="muted">Nenhuma especialidade ativa. Cadastre-a em Configuração operacional.</span> : null}</div></details></F>
  ...
  </div></section><div className="form-footer"><button className="btn btn-secondary" type="button" onClick={() => { setShowForm(false); setEditingId(null); setForm(EMPTY); }}>Cancelar</button><button className="btn btn-primary" disabled={submitting}><Save size={16} /> {submitting ? 'Salvando…' : editingId ? 'Salvar alterações' : 'Salvar fornecedor'}</button></div></form> : null}
  {loading ? <LoadingPanel /> : <section className="card table-card">{items.length ? <div className="table-wrapper"><table className="data-table"><thead><tr><th>Fornecedor</th><th>Tipo / áreas</th><th>Endereço</th><th>Contato</th><th>Contratos</th><th>OS</th><th>Sanções</th><th>Ações</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} onClick={() => setSelected(item)} style={{ cursor: 'pointer' }}><td><span className="table-primary">{item.tradeName || item.legalName}</span><span className="table-secondary">{item.taxId}</span></td><td><span className="table-primary">{item.kind === 'CONSORTIUM' ? 'Consórcio' : 'Empresa'}</span><span className="table-secondary">{item.serviceAreaLinks?.map((link) => link.category.name).join(', ') || 'Sem área'}</span></td><td><span className="table-primary">{item.city ? `${item.city}/${item.state ?? ''}` : '—'}</span><span className="table-secondary">{item.addressLine1 || item.postalCode || 'Não informado'}</span></td><td><span className="table-primary">{item.contactName || '—'}</span><span className="table-secondary">{item.email || item.phone || 'Sem contato'}</span></td><td>{item._count?.contracts ?? 0}</td><td>{item._count?.directWorkOrders ?? 0}</td><td>{item._count?.penalties ?? 0}</td><td><div className="table-actions"><button className="btn btn-ghost" type="button" onClick={(event) => { event.stopPropagation(); editSupplier(item); }}><Pencil size={15} /> Editar</button><button className="btn btn-ghost danger-text" type="button" onClick={(event) => { event.stopPropagation(); void archiveSupplier(item); }}><Trash2 size={15} /> Excluir</button></div></td></tr>)}</tbody></table></div> : <EmptyState icon={UsersRound} title="Nenhum fornecedor cadastrado" description="Cadastre empresas para vinculá-las a contratos e ordens de serviço." />}</section>}
